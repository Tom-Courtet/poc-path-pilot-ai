...
${data.availableLodgings && data.availableLodgings.length > 0
      ? data.availableLodgings.map(l => `- ID: ${l.$id} | Nom: ${l.lodgingName} | Lieu: ${l.location} | Prix/nuit: ${l.pricePerNight}€ | Capacité: ${l.numberOfGuests} pers. | Chambres: ${l.numberOfRooms}${l.wifi ? ' | WiFi' : ''}${l.clim ? ' | Clim' : ''}`).join("\n")
      : "Aucun logement disponible"}

## Transports disponibles
${data.availableTransports && data.availableTransports.length > 0
      ? data.availableTransports.map(t => `- ID: ${t.$id} | Type: ${t.type} | ${t.departureLocation} → ${t.arrivalLocation} | Horaires: ${t.departureHour} - ${t.arrivalHour} | Compagnie: ${t.company} | Prix: ${t.price}`).join("\n")
